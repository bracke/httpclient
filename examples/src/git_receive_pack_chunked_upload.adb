...
begin
   --  Caller supplies entity bytes only. HttpClient owns HTTP/1.1 chunk framing.
   ...
   if Status = Http_Client.Errors.Ok then
      Status := Http_Client.Requests.Set_Body
        (Request, Http_Client.Request_Bodies.From_Unknown_Length_Stream
          (Producer => Producer'Unchecked_Access, Replayable => False));
   end if;
   if Status = Http_Client.Errors.Ok then
...
end Git_Receive_Pack_Chunked_Upload;
