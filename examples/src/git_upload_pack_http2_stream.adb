...
procedure Git_Upload_Pack_HTTP2_Stream is
...
begin
   ...
   Options.Protocol_Policy := Http_Client.Response_Streams.Streaming_Prefer_HTTP_2;
   Options.Enable_Decompression := False;

   Status := Http_Client.Response_Streams.Open (Request, Stream, Options);
   if Status /= Http_Client.Errors.Ok then
      Ada.Text_IO.Put_Line ("stream open failed");
      return;
   end if;
...
end Git_Upload_Pack_HTTP2_Stream;
