...
procedure Git_Info_Refs_HTTPS_SOCKS_Stream is
...
begin
   ...
   Options.Proxy :=
     Http_Client.Proxies.SOCKS5
       (Host     => "socks.example",
        Port     => 1080,
        DNS_Mode => Http_Client.Proxies.SOCKS5_Remote_DNS);
   ...
   if Status = Http_Client.Errors.Ok then
      --  The streaming transport negotiates SOCKS5 CONNECT to example.invalid:443,
      --  starts origin TLS inside the SOCKS tunnel, verifies example.invalid, and
      --  sends Git headers only after the TLS handshake succeeds.
...
end Git_Info_Refs_HTTPS_SOCKS_Stream;
