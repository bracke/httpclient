...
package body Http_Client.HTTP3.Mapping is
   ...
   begin
      if Is_Pseudo (Name) then
         --  Header_List deliberately has one pseudo-header insertion helper.
         --  HTTP/3 uses the same syntactic validation while this package owns
         --  the protocol-specific semantic validation and status mapping.
      ...
      for I in 1 .. Http_Client.Headers.Length (Headers) loop
         ...
         begin
            ...
            if Is_Pseudo (Name) then
               ...
               else return Http_Client.Errors.HTTP3_Protocol_Error;
               end if;
            else
               Saw_Regular := True;
